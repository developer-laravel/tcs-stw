...
class TransferSchoolChoiceController extends Controller {
    use TransferControllerTraits;

    /**
     * Handles the School Choice Transfers
     *
     * @param Request $request
     * @param int     $step
     *
     * @Route( "/school-choice/step/{step}" , name="stw_sct" , requirements={ "step" = "\d+" } )
     * @return \Symfony\Component\HttpFoundation\Response;
     */
    public function SchoolChoiceTransferAction( Request $request , $step ){
        ...
        if( $openEnrollment == null ){
            $openEnrollment = $this->getOpenEnrollmentPeriod();
            $openEnrollment = ( $openEnrollment ) ? $openEnrollment[0] : null;
        }

        $message = $em->getRepository('IIABStudentTransferBundle:Settings')->findOneBy( array(
            'settingName' => 'school choice message'
        ) );
            ...
            case 4:
                ...
                if( $zonedSchools != false ) {
                    ...
                    if( $student->getGrade() > 90
                        ...
                        $zoned_school = $zonedSchools['Elementary'];
                        $maybe_choice_school = $zonedSchools['choiceElementary'];
                    } else if( $student->getGrade() < 8 ){
                        $zoned_school = $zonedSchools['Middle'];
                        $maybe_choice_school = $zonedSchools['choiceMiddle'];
                    } else if ( $student->getGrade() <= 12 ){
                        $zoned_school = $zonedSchools['High'];
                        $maybe_choice_school = $zonedSchools['choiceHigh'];
                    }

                    if( $maybe_choice_school ){
                        $next_grade = ( $student->getGrade() != 99 )
                            ? sprintf('%02d', $student->getGrade() + 1 )
                            : '00';

                        $maybe_choice_school = $em->getRepository( 'IIABStudentTransferBundle:ADM' )
                            ->findOneBy([
                                'hsvCityName' => $maybe_choice_school,
                                'grade' => $next_grade,
                            ]);
                        $zoned_school = $em->getRepository( 'IIABStudentTransferBundle:ADM' )
                            ->findOneBy([
                                'hsvCityName' => $zoned_school,
                                'grade' => $next_grade,
                            ]);
                    }

                    if( !empty( $maybe_choice_school )
                        && !empty( $zoned_school )
                     ){
                        ...
                        foreach( $all_schools as $id => $school ){
                            if( $maybe_choice_school->getId() == $id
                                || $zoned_school->getId() == $id
                            ){
                                $schools[$id] = $school;
                            }
                        }
        ...
        return $this->render( '@IIABStudentTransfer/Default/transferSchoolChoice.html.twig' , array(
