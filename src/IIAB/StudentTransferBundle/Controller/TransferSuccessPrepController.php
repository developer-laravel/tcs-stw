...
class TransferSuccessPrepController extends Controller {
    use TransferControllerTraits;

    /**
     * Handles the Success Prep Transfers
     *
     * @param Request $request
     * @param int     $step
     *
     * @Route( "/success-prep/step/{step}" , name="stw_success_prep" , requirements={ "step" = "\d+" } )
     * @return \Symfony\Component\HttpFoundation\Response;
     */
    public function successPrepTransferAction( Request $request , $step ) {
        ...
        $message = $em->getRepository('IIABStudentTransferBundle:Settings')->findOneBy( array(
            'settingName' => 'success prep message'
        ) );
        ...
        switch( $step ) {
                ...
                if( $lastFormData['studentStatus'] == 'currentStudent' ) {
                    ...
                    $form = $this->createFormBuilder()
                        ...
                        ->add( 'referral', 'choice', [
                            ...
                            'data' => isset( $lastFormData['referral'] ) ? $lastFormData['referral'] : '',
                        ...
                        ->add( 'gradesRepeated', 'choice', [
                            'label' => 'forms.gradesRepeated',
                            'choices' => [
                                'None' => 'none',
                                'K' => 'K',
                                '1st' => 1,
                            ...
                            'placeholder' => 'forms.selectOption',
                            'data' => isset( $lastFormData['gradesRepeated'] ) ? $lastFormData['gradesRepeated'] : [],
                            'multiple' => true,
                            'expanded' => true,
                        ])
                        ->add( 'submitAndNext' , 'submit' , array(
            ...
            case 3:
                ...
                $student = $em
                    ->getRepository( 'IIABStudentTransferBundle:Student' )
                    ->findOneBy( array(
                ...
                //$textFields[] = array( 'label' => $this->get( 'translator' )->trans( 'forms.nextGrade' , array( '%next%' => '('.$nextSchoolYearText->getSettingValue().')' ) , 'IIABStudentTransferBundle' ) , 'value' => $lastFormData['nextGrade'] );
                ...
                $repeatedGrades_text = implode( ', ', $lastFormData['gradesRepeated'] );
                $textFields[] = array( 'label' => $this->container->get( 'translator' )->trans( 'forms.gradesRepeated' , array() , 'IIABStudentTransferBundle' ) , 'value' => $repeatedGrades_text );
            ...
            case 4:
                ...
                $student = $em->getRepository( 'IIABStudentTransferBundle:Student' )->findOneBy(
                    array( 'studentID' => $lastFormData['studentID'] , 'dob' => $lastFormData['dob']->format( 'n/j/Y' ) ) );
                ...
                $is_passing = ( $student->getGrade() == 7 ) ? 'y' : 'n';
                ...
                $all_schools = ( $all_schools ) ? $all_schools : [];

                $schools = $this->limitSchoolsBySetting( $all_schools, 'Success Prep Schools' );
                ...
                if( $form->isValid() ) {
                    ...
                    $student = $em->getRepository( 'IIABStudentTransferBundle:Student' )
                        ->findOneBy( array( 'studentID' => $lastFormData['studentID'] ,
                                'dob' => $lastFormData['dob']->format( 'n/j/Y' )
                     ) );
                    $openEnrollments = $this->getOpenEnrollmentPeriod( $lastFormData['enrollment'] );
                    ...
                    if( $openEnrollments && $student != null && empty( $formData['choice3'] ) ) {
                        ...
                        if( $submission == null ) {
                            ...
                            if( isset( $lastFormData['gradesRepeated'] ) ){

                                $repeatedGrades_text = implode( ', ', $lastFormData['gradesRepeated'] );

                                $gradesRepeated = new SubmissionData();
                                $gradesRepeated->setMetaKey('grades_repeated');
                                $gradesRepeated->setMetaValue( $repeatedGrades_text );
                                $gradesRepeated->setSubmission( $submission );

                                $submission->addSubmissionDatum( $gradesRepeated );
                                $em->persist( $gradesRepeated );
                            }

                            if( isset( $lastFormData['referral'] ) ){

                                $referral = new SubmissionData();
                                $referral->setMetaKey('referral');
                                $referral->setMetaValue( $lastFormData['referral'] );
                                $referral->setSubmission( $submission );

                                $submission->addSubmissionDatum( $referral );
                                $em->persist( $referral );
                            }
        ...
        return $this->render( '@IIABStudentTransfer/Default/transferSuccessPrep.html.twig' , array( 'form' => $form->createView() , 'step' => ( $step - 1 ) , 'nonFormFields' => $textFields , 'openEnrollment' => $openEnrollment, 'message' => $message ) );
